...
void st7789_init(const struct st7789_config* config, uint16_t width, uint16_t height)
{
    ...
    dma_chno = dma_claim_unused_channel(true);
}
...
void st7789_blit_screen(int pixels, uint16_t* buf)
{
    ...
    dma_channel_config cfg = dma_channel_get_default_config(dma_chno);
    ...
    dma_channel_configure(dma_chno, &cfg,
    ...
    while (dma_channel_is_busy(dma_chno)) {
        sleep_us(100);
    }
}

bool st7789_blit_done()
{
    return !dma_channel_is_busy(dma_chno);
}
