...
void update_radar()
{
	...
	if ((time_us_64() - last_report_time) > (1000*1000) && (time_us_64() - last_reinit_time) > (1000*1000) && get_switched_12v())
	{
		init_radar_comms();
...
void radar_debug()
{
	...
	dbgf("Radar: DD: %dcm PIN:%d/%d\n", last_report.report.detection_distance_cm, gpio_get(PIN_MMWAVE_RX), gpio_get(PIN_MMWAVE_TX));
