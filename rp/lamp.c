...
void lamp_update(void)
{
	...
	// Fault check: 12V out of range → emergency shutdown.
	// V1.2: gate on b_lamp_is_12v_on — during the type test the buck may be
	// intentionally off and g_sense_12v ≈ 0V is expected, not a fault.
	// V1.1: 12V rail is always present, so the check always applies.
	bool b_check_12v = board_is_v1_2() ? b_lamp_is_12v_on : true;

	if (b_check_12v && ((g_sense_12v < 10.5) || (g_sense_12v > 13.5)))
	{
...
void lamp_set_switched_12v(bool b_on)
{
	if (b_on)
	{
		if (board_is_v1_2())
		{
			// V1.2: Can't pre-check 12V — it's generated by the buck we're enabling.
			// Require 24V (the source) to be on first.
			if (!b_lamp_is_24v_on)
			{
				printf("Reject 12V enable: 24V not on\n");
				return;
			}
		}
		else
		{
			// V1.1: 12V rail comes from barrel/USB — check it's in range before switching
	...
	if (!b_lamp_is_12v_on && b_on)
	{
		...
		if (board_is_v1_2())
		{
			// V1.2: Post-enable verification (12V now exists from buck)
			sleep_ms(50);
			sense_update();
			...
			if (g_sense_12v < 10.5 || g_sense_12v > 13.5)
			{
				...
				return; // b_lamp_is_12v_on stays false
			}
		}
...
void lamp_set_switched_24v(bool b_on)
{
	if (board_is_v1_2())
	{
		...
		{
			printf("Reject 24V disable: 12V still on (depends on 24V)\n");
			return;
		}
	}
	else
	{
		// V1.1: 24V boost needs 12V as input — reject enable without 12V
		if (!b_lamp_is_12v_on && b_on)
		{
...
bool lamp_request_power_level(LAMP_PWR_LEVEL_E pwr_level)
{
	...
	// Both rails required (V1.2: 24V is upstream of 12V; V1.1: 24V needed for non-dimmable)
	if (!b_lamp_is_24v_on && pwr_level != LAMP_PWR_OFF_C)
	{
		if (board_is_v1_2() || lamp_get_type() == LAMP_TYPE_NON_DIMMABLE_C)
		{
